...
レビュー結果を表示する画面。一括レビュー・分割レビューの両方で共通の結果画面を使用する。
...
結果画面はTraceability Matrixテーブルのセル内容が長くなるため、メイン画面・実行中画面の `max-w-4xl`（896px）ではなくフル幅コンテナを使用する。
各セクションの横幅は、最も幅の広いセクション（Traceability Matrixテーブル）に揃える。テーブル内容が短い場合でも最低 `max-w-4xl` 相当の幅を確保する。

- 結果画面全体を `w-fit`（内容に応じた幅）で囲み、`max-w-full`（画面幅上限）・`min-w-[56rem]`（最低幅）を設定
- ヘッダー・Traceability Matrix・実行情報・詳細レポート・ZIP一式ダウンロードの全セクションが同一幅で表示される

...
あなたは設計書とプログラムコードを紐付けし、整合性を検証するレビュアーです。
...
1. サマリー（紐付け日時、ファイル名、総合判定）
2. 紐付け結果一覧（テーブル形式）
3. 詳細（問題点と推奨事項）

#### notes

- メイン設計書の内容について紐付けしてください。
  ...
  sections: [
    ...
    },
    {
      ...
      defaults: [
        {
          name: '標準レビュープリセット',
          role: 'あなたは設計書とプログラムコードを紐付けし、整合性を検証するレビュアーです。',

...
設計書（Excel/Markdown形式）とプログラムコードをAIで紐付け、ソースコード間の「トレーサビリティ（追跡可能性）」を可視化・管理するWebアプリケーション。
紐付け結果をトレーサビリティ・マトリクスとして表示し、Markdown形式で出力する。

### 1.2 解決する課題

- 設計と実装の紐付け作業の自動化・効率化
- 仕様変更時の影響範囲の特定を迅速化
- 設計書とコードの不整合（乖離）の早期発見
...
| 値 | 説明 |
|-----|------|
| メイン | AIが紐付けレビューの主対象とする設計書（1つのみ選択可能） |
...
| 設計書とソースのマッピング | このマッピングに基づいて紐付けを行ってください |
...
| 項目 | 説明 | デフォルト値 |
|------|------|-------------|
| 役割 | AIの役割定義 | あなたは設計書とプログラムコードを紐付けし、整合性を検証するレビュアーです。 |
| 目的 | タスク内容とレビュー観点 | （後述） |
| 出力形式 | 出力フォーマット指定 | （後述） |
| 注意事項 | 制約・判断基準 | （後述） |
...
```
メイン設計書とプログラムを紐付けして、設計書の内容がプログラムに正しく実装されているかを検証し、差異や問題点を報告してください。
...
変換済みの設計書（Markdown）とプログラム（行番号付き）、およびシステムプロンプトをLLMに送信し、紐付けレビューを実行する。
...
```markdown
# 設計書-Javaプログラム紐付け AIマッパー レビューレポート
...
大規模なファイルの場合、AIの入力トークン上限を超えてレビューを実行できないケースがある。この問題を解決するため、md2map / code2map を使用してファイルを意味のある単位（セクション / クラス・関数）に分割し、逐次レビューを実行する機能を提供する。

#### 2.7.1 分割ツール

| ツール | 対象 | 分割単位 |
|--------|------|----------|
| [md2map](https://github.com/elvezjp/md2map) | Markdown設計書 | 見出し（H1-H6）単位 |
| [code2map](https://github.com/elvezjp/code2map) | ソースコード | クラス/メソッド/関数単位 |

#### 2.7.2 分割モード

以下の4つのモードをサポートする。ユーザーが明示的に分割モードを選択する（自動判定は行わない）。
...
全グループのレビュー結果を統合し、最終的なレビューレポートを生成する。
...
レビュー実行前に、分割結果をプレビューできる。プレビューには以下の情報が表示される。
...
グループレビュー中にエラーが発生した場合:

- 処理が一時停止し、エラーが発生したグループを表示
- ユーザーは「リトライ」または「スキップ」を選択可能
- スキップした場合、そのグループは最終レポートから除外される
...
| openapi | OpenAPI/REST API レビュー | REST API仕様と実装の紐付け向け | API, OpenAPI, REST |
...
| batch-etl | バッチ/ETL処理 レビュー | バッチ処理・ETLジョブの設計書紐付け向け | バッチ, ETL, データ処理 |
...
2. **設計書セクション**
   ...
   - 注記テキスト: 「※ メイン設計書を対象に紐付けし、それ以外は必要に応じて参照されます。種別ごとの注意事項が変換後マークダウンに付与されます。特別な処理を指示したい場合はシステムプロンプトを編集してください。」
   - 「マークダウンに変換」ボタン
...
5. **推定入力トークン数表示**
   - 設計書・プログラムの変換完了後に表示
   - 設計書・プログラム別の内訳と合計を表示
   - 文字数から推定トークン数を算出（算出方法: `日本語文字数 × 1.5 + その他文字数 × 0.25`）
     - 日本語: 約1.5トークン/文字（ひらがな、カタカナ、漢字）
     - その他: 約0.25トークン/文字（英数字、記号、スペース等）
     - 算出方法を表示して概算であることを説明
   - 100,000トークン以上の場合は警告表示: 「トークン数が多いため、処理に時間がかかったり、エラーになる可能性があります。」

6. **分割設定セクション**
   - 設計書・プログラムが大きくトークン上限を超える場合に分割レビューを選択可能
   - 設計書: 一括 / 分割（md2mapで見出し単位に分割）
   - プログラム: 一括 / 分割（code2mapでクラス・関数単位に分割）
   - 分割オプション（分割選択時に展開）
     - 設計書: 見出しレベル（H2まで（推奨）/ H3まで / H4まで）
     - プログラム: 対応言語表示（Python / Java）、未対応ファイルの警告
   - 「分割プレビュー実行」ボタン
   - プレビュー結果表示（設計書パーツ一覧、コードパーツ一覧）

7. **レビュー実行セクション**
   - 現在のモード表示（一括 / 分割）
   - 分割時: パート数、推定レビュー実行回数を表示
   - 分割選択時は「分割プレビュー実行」が必須（未実行時はレビュー実行ボタン無効）

8. **レビュー実行ボタン**

### 3.2 ローディング画面（一括レビュー）

一括レビュー実行中に表示。スピナーと「AIがレビューを実行中...」メッセージを表示。

### 3.3 分割レビュー実行画面

分割レビュー実行中に表示する専用画面。3フェーズの進捗をリアルタイムで表示する。
...
3. **実行ステップ詳細**
   - **1. 構造マッチング**
     - ステータス表示（完了/実行中/待機中）
     - 完了時: マッチング結果（グループ数）を表示
   - **2.x グループレビュー**（グループごとに表示）
     - グループ名
     - 対象の設計書セクション一覧
     - 対象のコードシンボル一覧
     - ステータス表示（完了/実行中/待機中/エラー/スキップ）
     - 完了時: レビュー結果のプレビュー（折りたたみ）
     - エラー時: エラーメッセージ、「リトライ」「スキップ」ボタン
   - **3. 結果統合**
     - ステータス表示
     - 完了時: 総合評価サマリーを表示

4. **操作ボタン**
   - エラー発生時: 「再開（エラーをスキップ）」ボタン
...
マッピング結果を表示する画面。構造マッチングAPIの実行結果を一覧表示し、各種ダウンロード機能を提供する。
...
2. **マッピング結果一覧**
   - サマリ情報: グループ数、設計書セクション数（ユニーク）、コードシンボル数（ユニーク）
   - テーブル形式で一覧表示
   ...
   | 項番 | 連番（1, 2, 3, ...） |
   | グループ名 | マッチンググループの名称 |
   | 設計書セクション | セクションID（バッジ表示）+ タイトル。複数の場合は縦に並べる |
   | コードシンボル | `ファイル名 :: シンボル名 (L開始行-終了行)`。行範囲は分割プレビュー結果と突合して表示。IDの突合に失敗した場合は行範囲なしで表示 |
   | 理由 | マッチングの理由 |

   - テーブル下部に「CSVダウンロード」ボタン（全幅、グレー）
     - BOM付きUTF-8のCSVファイル（`mapping-result.csv`）をダウンロード
     - CSVの列構成はテーブルと同一（項番, グループ名, 設計書セクション, コードシンボル, 理由）
     - 複数のセクション・シンボルはセミコロン区切りで結合

3. **マッピング結果レポート**
   - 見出し: 「マッピング結果レポート」
   - 説明文: 「マッピング結果一覧の内容をマークダウン形式で出力したレポートです。」
   - マッピング結果のレポートテキスト（プレーンテキスト形式、等幅フォント）
   - コピーボタン / ダウンロードボタン（横並び）
   - ダウンロードファイル名: `mapping-result-report.md`（セクション形式のレポート + テーブル形式の Traceability Matrix を1ファイルに統合）
...
5. **マッピング実行データ一式ダウンロード**
   - 説明テキスト: 「マッピング実行の入出力データを一式ダウンロードできます。」
   - ダウンロード内容一覧（テーブル形式）
   - 「一式ダウンロード（ZIP）」ボタン（全幅、緑色）
...
├──────────────────────────────────────────────────────────────────────────────┤
│ ■ マッピング結果一覧                                                        │
│   グループ数: 5   設計書セクション: 12   コードシンボル: 8                    │
│                                                                              │
│   ┌────┬──────────┬──────────────────┬───────────────────────────┬────────┐ │
│   │項番│グループ名 │設計書セクション    │コードシンボル              │理由    │ │
│   ├────┼──────────┼──────────────────┼───────────────────────────┼────────┤ │
│   │ 1  │ユーザー   │[MD1] ユーザー管理 │main.py :: UserService     │ユーザー│ │
│   │    │管理       │[MD2] 認証処理     │  (L47-120)                │管理と  │ │
│   │    │          │                  │auth.py :: AuthManager      │認証を  │ │
│   │    │          │                  │  (L10-85)                  │統合    │ │
│   ├────┼──────────┼──────────────────┼───────────────────────────┼────────┤ │
│   │ 2  │データ     │[MD3] DB設計       │db.py :: DatabasePool      │...     │ │
│   │    │アクセス   │                  │  (L5-60)                   │        │ │
│   └────┴──────────┴──────────────────┴───────────────────────────┴────────┘ │
│                                                                              │
│   [                        CSVダウンロード                                 ] │
...
│   │ system-prompt.md             │ システムプロンプト（役割・目的・        │    │
│   │                              │ 出力形式・注意事項）                    │    │
│   │ spec-markdown.md             │ 変換後の設計書（マークダウン形式）      │    │
│   │ code-numbered.txt            │ 行番号付きプログラム                    │    │
│   │ mapping-result-report.md     │ マッピング結果レポート（セクション形式  │    │
│   │                              │ + テーブル形式）                        │    │
...
コードシンボルの行範囲は、フロントエンド側で分割プレビュー結果（`codeParts`）とマッピング結果（`codeSymbols`）のIDを突合して取得する。

- 分割プレビュー時に得られた `CodePart` の `id`、`startLine`、`endLine` を `CodeLineMap`（`Map<string, { startLine, endLine }>`）として保持
- マッピング結果の各 `MatchedCodeSymbol.id` で `CodeLineMap` を検索し、一致すれば `(L開始行-終了行)` を付記
- IDが一致しない場合（AIが存在しないIDを返した場合など）は、行範囲なしでシンボル名のみ表示
- CSV・Markdownエクスポートにも同様に行範囲を反映

**横幅の制御:**

結果画面はマッピング結果一覧テーブルのセル内容が長くなるため、メイン画面・実行中画面の `max-w-4xl`（896px）ではなくフル幅コンテナを使用する。
各セクションの横幅は、最も幅の広いセクション（マッピング結果一覧テーブル）に揃える。テーブル内容が短い場合でも最低幅を確保する。

- 結果画面全体を `w-fit`（内容に応じた幅）で囲み、`max-w-full`（画面幅上限）・`min-w-[56rem]`（最低幅）を設定
- ヘッダー・マッピング結果一覧・マッピング結果レポート・実行情報・ZIP一式ダウンロードの全セクションが同一幅で表示される
...
3. **プリセット一覧**
   ...
   - 各カードの構成:
     ...
     - 説明文
     - タグ（ピル形式、複数表示）
     - 「システムプロンプトを見る」（折りたたみ、クリックで展開）
       - マークダウン形式で役割、目的、フォーマット、注意事項を表示
       - 改行が保持されるプレビュー形式（max-h-64でスクロール可能）
     - 「設計書種別と注意事項を見る」（折りたたみ、クリックで展開）
       - テーブル形式で種別と注意事項を表示
     - 「このプリセットを使う」ボタン
...
3. **システムプロンプト表示**: 「システムプロンプトを見る」をクリックで展開/折りたたみ（マークダウン形式）
4. **設計書種別表示**: 「設計書種別と注意事項を見る」をクリックで展開/折りたたみ（テーブル形式）
5. **適用**: 「このプリセットを使う」クリックでプリセットを適用し、メイン画面に戻る
   - システムプロンプトと設計書種別の両方が更新される
6. **適用後のトースト**: メイン画面に「プリセット「{名前}」を適用しました」と表示
...
```
┌─────────────────────────────────────────────────────────────┐
│  ( v0.1.0 )       設計書-Javaプログラム紐付け AIマッパー     │
│                                    📖 プリセット  [⚙️]      │
└─────────────────────────────────────────────────────────────┘
```
...
| POST | `/api/review/group` | グループレビュー（分割レビュー フェーズ2） |
...
| フィールド | 必須 | 説明 |
|-----------|------|------|
| content | ○ | 分割対象のコード（行番号なし） |
| filename | ○ | ファイル名（拡張子で言語判定） |
...
```json
{
  ...
  "parts": [
    ...
    },
    {
      ...
      "startLine": 5,
      "endLine": 20,
      "content": "public void createUser() {...}",
      "estimatedTokens": 400
    }
  ],
  "indexContent": "# INDEX\n\n- CD1: UserService (class, L1-L50)\n...",
  "language": "java"
}
```

#### POST /api/review/structure-matching

構造マッチング（分割レビュー フェーズ1）。設計書とコードの構造を比較し、関連性の高いグループを特定する。
...
```json
{
  "document": {
    ...
    "mapJson": {
      "sections": [
        {"id": "MD1", "title": "ユーザー管理", "level": 2, "path": "ユーザー管理", "startLine": 1, "endLine": 50}
      ]
    }
  },
  "codeFiles": [
    {
      ...
      "mapJson": {
        "symbols": [
          {"id": "CD1", "name": "UserService", "symbolType": "class", "startLine": 1, "endLine": 100}
        ]
      }
    }
  ],
...
```json
{
  ...
  "tokensUsed": {"input": 3000, "output": 1000}
}
```

#### POST /api/review/integrate

結果統合（分割レビュー フェーズ3）。全グループのレビュー結果を統合し、最終レポートを生成する。
...
```json
{
  ...
  "report": "# 設計書-Javaプログラム紐付け AIマッパー レビューレポート\n\n## レビュー情報\n\n...",
  "integratedReport": {
    "overallSummary": "レビュー対象: 3グループ",
    "consistencyScore": 0.85,
    "keyIssues": [],
    "crossGroupIssues": [],
    "statistics": {"totalGroupsReviewed": 3},
    "deduplicatedFindings": []
  },
  "reviewMeta": {
    "version": "v0.1.0",
    "modelId": "claude-haiku-4-5-20251001",
    "executedAt": "2026/02/06 14:30",
    "designs": [],
    "programs": [],
    "inputTokens": 5000,
    "outputTokens": 2000
...
[ユーザー]
    ...
    ├─(3) 分割設定
    │         │
    │         ├─ 設計書: 一括 or 分割（見出しレベル選択）
    │         └─ プログラム: 一括 or 分割
    │
    ├─(4) 分割プレビュー実行
    │         │
    │         ├─ 設計書分割: /api/split/markdown
    │         │     ↓
    │         │   md2mapでセクション分割
    │         │     ↓
    │         │   DocumentPart[] + INDEX.md
    │         │
    │         └─ コード分割: /api/split/code（ファイルごと）
    │               ↓
    │           code2mapでシンボル分割
    │               ↓
    │           CodePart[] + INDEX.md
...
| Markdown分割 | md2map (elvezjp) |
| コード分割 | code2map (elvezjp) |
| AWS SDK | boto3 |
...
│   │   │   ├── reviewer/        # レビュー機能
│   │   │   │   ├── components/
│   │   │   │   │   ├── SplitSettingsSection.tsx  # 分割設定セクション
│   │   │   │   │   ├── SplitExecutingScreen.tsx  # 分割レビュー実行画面
│   │   │   │   │   └── ExecutionInfo.tsx         # 実行情報表示
│   │   │   │   ├── hooks/
│   │   │   │   │   └── useSplitSettings.ts       # 分割設定フック
│   │   │   │   ├── services/
│   │   │   │   └── types/
...
│       │   ├── __init__.py
│       │   ├── convert.py       # 変換API
│       │   ├── review.py        # レビューAPI（一括・分割統合）
│       │   └── split.py         # 分割API（md2map/code2map）
│       ├── services/
│       │   ├── __init__.py
...
| 項目 | 変更内容 |
|------|----------|
| 新機能 | セマンティック分割レビュー機能を追加 |
| 分割ツール | md2map（Markdown分割）、code2map（コード分割）を統合 |
| 分割API | `/api/split/markdown`、`/api/split/code` を追加 |
| レビューAPI | `/api/review/structure-matching`、`/api/review/group`、`/api/review/integrate` を追加 |
| UI | 分割設定セクション、分割レビュー実行画面を追加 |
...
| 用語 | 定義 |
|------|------|
| 紐付け | 設計書とコードを照らし合わせて整合性を確認すること |
| 設計書 | システムの機能・仕様を記述したドキュメント |
| レビュー観点 | 紐付け時にチェックする項目のカテゴリ |
...
| 分割API | backend/app/routers/split.py |
| 分割レビューAPI | backend/app/routers/review.py（構造マッチング・グループレビュー・統合） |

#### 13.2.2 フロントエンド テスト対象モジュール
...
| ID | 関数 | テスト内容 | 期待結果 |
|----|------|----------|---------|
| UT-BED-PROVIDER-001 | BedrockProvider.**init**() | IAMロール認証（accessKeyId/secretAccessKeyがNone） | 認証情報なしでboto3クライアント作成 |
| UT-BED-PROVIDER-002 | BedrockProvider.**init**() | ユーザー指定認証情報 | 認証情報付きでboto3クライアント作成 |
...
| E2E-RV-008 | レポートヘッダ確認 | レビュー完了済み | ダウンロードしたMDファイルを確認 | 「設計書-Javaプログラム紐付け AIマッパー レビューレポート」タイトルとレビュー情報テーブルが含まれる | |
...
| # | 試験項目 | 前提条件 | 操作手順 | 期待結果 | 結果 |
|---|---------|---------|---------|---------|------|
| E2E-VS-001 | バージョンボタン表示 | サーバー起動済み | ページにアクセス | 左上にピル型の「v0.1.0」ボタンが表示される | |
| E2E-VS-002 | バージョン切替 | v0.1.0表示中 | バルーンからv0.5.2をクリック | トースト表示後、ページリロード、v0.5.2のUIが表示される | |
| E2E-VS-003 | Cookie永続化確認 | E2E-VS-002完了 | ブラウザを閉じて再アクセス | v0.5.2が表示される（Cookieが保持されている） | |
| E2E-VS-004 | 無効なCookie時のフォールバック | - | DevToolsでCookieを「v9.9.9」に設定してアクセス | v0.1.0（デフォルト）が表示される | |

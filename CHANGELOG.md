...
### Changed
- **[BREAKING] Retired the `versions/` directory and moved to a layout that keeps only the latest code at the repository root** (#16): The repository kept a full snapshot of every release (v0.1.0 / v0.1.1 / v0.1.2) under `versions/`, which caused duplicate Dependabot alerts against the lockfiles of old versions and an out-of-scope triage on every security fix. `versions/v0.1.2/backend` and `frontend` were promoted to `backend/` and `frontend/` at the root, and `spec.md` / `config-file-generator-spec.md` moved to `docs/`. `versions/v0.1.0`, `v0.1.1`, and `versions/README.md` were removed, and the CI `working-directory` now points at the root layout. The old layout (`versions/` plus the in-tree copies) is preserved under the existing `v0.1.2` tag and can be inspected with `git checkout v0.1.2` (no per-version tags are created retroactively). Going forward, changes for the next release accumulate on main and a `vX.Y.Z` tag is created at release time
- **Updated the documentation for the single-version layout and git-tag based releases** (#16, #20): The launch and test steps in README / CONTRIBUTING now use the root layout (`cd backend` / `cd frontend`), and README gained a "Version Management" section describing how to inspect the old layout with `git checkout v0.1.2`. It also states that the code under the `v0.1.2` tag is a frozen snapshot that does not include this release's security fixes and is for reference and verification only. "Related Projects" in README was rewritten from the git-subtree wording to describe the tools as uv dependencies, pointing to the upstream repositories for anyone who needs the sources. The Dependabot alert policy in README / SECURITY dropped the wording that assumed dismissing alerts for old versions and git subtree directories, and now covers the root lockfiles only. `docs/spec.md` lost the "version-switching UI" chapter and the related E2E test items (version switching, old-version checks), and had its broken links and outdated excel2md version references fixed
- **Updated the backend dependencies**: regenerated `backend/uv.lock` with `uv lock --upgrade`, updating 31 packages (`anthropic` 0.109.2 → 0.121.0, `openai` 2.42.0 → 2.53.0, `fastapi` 0.137.1 → 0.141.1, `starlette` 1.3.1 → 1.6.0, `uvicorn` 0.49.0 → 0.52.1, `pandas` 3.0.3 → 3.0.5, `markitdown` 0.1.6 → 0.1.7, `tree-sitter` 0.25.2 → 0.26.0, and others). Not a response to a specific advisory — a routine refresh
...
- **[BREAKING] Migrated `excel2md` from `sys.path` injection of an in-tree copy to a PyPI dependency** (#20): `excel2md_tool.py` / `excel2md_mermaid_tool.py` loaded the in-tree copy `excel2md/v2.1.1` by injecting it into `sys.path` at runtime; after #27 pinned the other in-house tools to tagged git references, `excel2md` was the only package left outside the dependency management in `pyproject.toml`. `excel2md>=2.2.1` is now declared in `dependencies` (published on PyPI, so no `[tool.uv.sources]` entry is needed; the lock resolves to 2.3.0), and the tools use ordinary imports of `excel2md.cli.build_argparser` / `excel2md.runner.run`. As a result, the `EXCEL2MD_PATH` environment variable, which pointed the backend at an excel2md outside the tree, has been removed. excel2md moves from v2.1.1 to v2.3.0, picking up the v2.2.x fixes and the v2.3.0 performance improvements. One difference in conversion output: **links to images placed outside the print area (or the used range when no print area is set) are no longer emitted into the CSV Markdown** (upstream v2.2.1 fix [excel2md#14](https://github.com/elvezjp/excel2md/issues/14); such images used to stretch the output range and drag in unrelated cells). All other sheets (tables, merged cells, multiple tables, hyperlinks) and Mermaid output were verified to match before and after the migration, apart from the generation timestamp line, using the in-tree test fixtures. All 194 tests pass

### Removed
- **[BREAKING] Retired the version-switching UI** (#16): Removed the pill-shaped version button and balloon at the top left of the screen (`VersionSelector` / `useVersions` / `DEFAULT_VERSIONS` / `VersionInfo`) together with the reads and writes of the `app_version` cookie. The feature was inherited from the AI Reviewer, but this repository has no infrastructure (nginx or similar) that routes to a per-version backend/frontend based on the cookie, and the only choice offered was the current version, so it never did anything. The running version is still shown in the settings modal as before. An `app_version` cookie left in the browser is no longer read and is harmless
- **[BREAKING] Removed the five external-tool directories that were embedded via git subtree** (#16, #20): `add-line-numbers/`, `code2map/`, `excel2md/`, `markitdown/`, and `md2map/` (779 files in total) were removed. uv installs `markitdown` and `excel2md` from PyPI and `add-line-numbers`, `code2map`, and `md2map` as git dependencies pinned to upstream release tags, so nothing referenced the embedded copies any more. Keeping them made upstream updates impossible to pick up, and Dependabot treated the manifests inside them (including the lockfiles of old upstream versions under `code2map/versions/` and `md2map/versions/`) as independent dependency sets, causing duplicate alerts. The dependency manifests in the repository went from 33 files to 4 (`backend/pyproject.toml`, `backend/uv.lock`, `frontend/package.json`, `frontend/package-lock.json`). To read the sources, clone the upstream repositories

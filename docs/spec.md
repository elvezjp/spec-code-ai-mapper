...
excel2mdをCSVマークダウン出力モードで実行する。概要セクションは出力し、検証用メタデータは出力しない（`--no-csv-include-metadata`）。
...
[ユーザー]
    ...
    │         │     ↓
    │         │   md2mapでセクション分割（heading/nlp/aiモード）
    │         │     ↓
    │         │   DocumentPart[] + INDEX.md + MAP.json
    │         │
    │         └─ コード分割: /api/split/code（ファイルごと）
    │               ↓
    │           code2mapでシンボル分割
    │               ↓
    │           CodePart[] + INDEX.md + MAP.json
    │
    └─(5) マッピング実行
              │
              ▼
...
#### 5.5.1 設計書分割（`/api/split/markdown`）
...
1. リクエストを受信
   - content: Markdown文字列
   - filename: ファイル名
   - maxDepth: 分割深度（デフォルト: 2 = H2まで）
   - splitMode: 分割モード（heading / nlp / ai）
   - llmConfig: LLM設定（aiモード時のみ使用）

2. 一時ファイルにMarkdownを書き込み

3. MarkdownParserでパース
   - split_mode に応じてパーサーを初期化
   - aiモードの場合は llmConfig を変換して渡す
   - sections, warnings = parser.parse(input_path, maxDepth)

4. セクションIDの割り当て
   - 各セクションに MD1, MD2, ... のIDを付与

5. パーツ・INDEX.md・MAP.jsonを生成
   - md2mapのgeneratorsを使用

6. レスポンスを返却
   - parts: DocumentPart[]（id, section, level, startLine, endLine, content）
   - indexContent: INDEX.md の内容
   - mapJson: MAP.json の内容
```

#### 5.5.2 コード分割（`/api/split/code`）
...
3. code2mapのパーサーでAST解析
   - クラス、メソッド、関数を抽出

4. シンボルIDの割り当て
   - 各シンボルに CD1, CD2, ... のIDを付与

5. INDEX.md・MAP.jsonを生成

6. レスポンスを返却
   - parts: CodePart[]（id, symbol, symbolType, startLine, endLine, content）
   - indexContent: INDEX.md の内容
   - mapJson: MAP.json の内容
```

### 5.6 構造マッチング処理（`/api/review/structure-matching`）

```python
# 処理の流れ（疑似コード）

1. リクエストを受信
   - document: { indexMd, mapJson }（設計書のメタデータ）
   - codeFiles: [{ filename, indexMd, mapJson }]（コードのメタデータ）
...
2. システムプロンプトを組み立て
   system_prompt = f"""
   ## 役割
   {role}    # デフォルト: "設計書とソースコードの構造を分析する専門家"

   ## 目的
   {purpose}  # デフォルト: "関連性の高い設計書セクションとコードシンボルをグループにまとめる"

   ## 出力形式
   {format}   # JSON形式（groups配列）

   ## 注意事項
   {notes}    # ID正確使用、多対多マッピング等の指示
   """

3. ユーザーメッセージを構築（メタデータのみ、実コンテンツは含まない）
   user_message = f"""
   ## 設計書構造
   ### INDEX.md
   {document.indexMd}
...
4. LLM APIを呼び出し
   - システムプロンプト + ユーザーメッセージを送信

5. JSON応答をパース
   - groups 配列から MatchedGroup[] を構築
   - 各グループ: { groupId, groupName, docSections, codeSymbols, reason }

6. レスポンスを返却
   - groups: MatchedGroup[]
   - totalGroups: グループ数
   - tokensUsed: { input, output }
   - reviewMeta: メタ情報（モデルID、トークン数等）
```
...
3. `docs/spec.md` 冒頭のバージョン番号を手動で更新
4. リリース時は CHANGELOG の見出しの日付を確定し、`vX.Y.Z` タグを作成する

※ バージョン管理は git tag で行い、リポジトリには最新コードのみを保持する。旧バージョンの参照方法は [README.md](../README.md) の「バージョン管理」を参照
...
試験結果は [`docs/tests/`](tests/) ディレクトリに日付・通し番号付きファイルで保存する。

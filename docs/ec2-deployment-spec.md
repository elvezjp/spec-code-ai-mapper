...
本ドキュメントは、設計書-コード紐付け管理 AI マッパー（spec-code-ai-mapper）のEC2デプロイに関する仕様を定義する。
...
└── nginx/
    ├── spec-code-ai-mapper.conf     # nginx server設定
...
|-----------|-------|--------|--------------|
| v0.5.1 (latest) | 8051 | 8000 + 5×10 + 1 | spec-code-ai-mapper-v0.5.1 |
| v0.5.0 | 8050 | 8000 + 5×10 + 0 | spec-code-ai-mapper-v0.5.0 |
...
# リポジトリのクローン（SSH URL）
sudo -E git clone git@github.com:elvezjp/spec-code-ai-mapper.git
sudo chown -R ubuntu:ubuntu spec-code-ai-mapper
```

### 5.3 バックエンドのセットアップ

```bash
cd /var/www/spec-code-ai-mapper

# 各バージョンの依存関係をインストール
cd versions/v0.2.5/backend
uv sync

cd /var/www/spec-code-ai-mapper/versions/v0.1.1/backend
uv sync

# 動作確認（最新版）
cd /var/www/spec-code-ai-mapper/latest/backend
...
const VERSIONS = [
  { name: 'spec-code-ai-mapper-v0.5.1', cwd: 'versions/v0.5.1', port: 8051, workers: 1 },
  { name: 'spec-code-ai-mapper-v0.5.0', cwd: 'versions/v0.5.0', port: 8050, workers: 1 },
  { name: 'spec-code-ai-mapper-v0.4.0', cwd: 'versions/v0.4.0', port: 8040, workers: 1 },
  { name: 'spec-code-ai-mapper-v0.3.0', cwd: 'versions/v0.3.0', port: 8030, workers: 1 },
  { name: 'spec-code-ai-mapper-v0.2.5', cwd: 'versions/v0.2.5', port: 8025, workers: 1 },
  { name: 'spec-code-ai-mapper-v0.1.1', cwd: 'versions/v0.1.1', port: 8011, workers: 1 },
];

// 共通設定
const BASE_PATH = '/var/www/spec-code-ai-mapper';
const commonConfig = {
  script: 'uv',
  interpreter: 'none',
  env: {
...
| `pm2 logs` | 全プロセスのログ表示 |
| `pm2 logs spec-code-ai-mapper` | 特定プロセスのログ表示 |
| `pm2 status` | ステータス確認 |
| `pm2 monit` | モニタリング |
...
# 設定ファイルのコピー
sudo cp /var/www/spec-code-ai-mapper/nginx/version-map.conf /etc/nginx/conf.d/
sudo cp /var/www/spec-code-ai-mapper/nginx/spec-code-ai-mapper.conf /etc/nginx/sites-available/

# シンボリックリンク作成
sudo ln -s /etc/nginx/sites-available/spec-code-ai-mapper.conf /etc/nginx/sites-enabled/
...
map $cookie_app_version $frontend_root {
    "v0.1.1"  /var/www/spec-code-ai-mapper/versions/v0.1.1/frontend;
    default   /var/www/spec-code-ai-mapper/latest/frontend;
}
```

**server設定（nginx/spec-code-ai-mapper.conf）:**
...
1. **IAMロールを作成**
   ...
   - ロール名: `[日付]spec-code-ai-mapper-ec2`

2. **カスタムポリシーをアタッチ**
   - ポリシー名: `[日付]spec-code-ai-mapper-ec2-allow-bedrock-invoke-model`
...
# 出力例
{
    ...
    "Arn": "arn:aws:sts::123456789012:assumed-role/[日付]spec-code-ai-mapper-ec2/i-0123456789abcdef0"
}
```
...
# 特定プロセスのログ
pm2 logs spec-code-ai-mapper
pm2 logs spec-code-ai-mapper-v0.1.1

# ログファイル直接参照
tail -f /var/log/pm2/spec-code-ai-mapper-out.log
tail -f /var/log/pm2/spec-code-ai-mapper-error.log
